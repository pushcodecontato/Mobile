...
var sql    = 'SELECT * FROM users WHERE id_tipo_veiculo = ' + SqlString.escape(userId);
console.log(sql); // SELECT * FROM users WHERE id_tipo_veiculo = 'some user provided value'
```
...
```js
var userId = 1;
var sql    = SqlString.format('SELECT * FROM users WHERE id_tipo_veiculo = ?', [userId]);
console.log(sql); // SELECT * FROM users WHERE id_tipo_veiculo = 1
```
...
```js
var userId = 1;
var sql    = SqlString.format('UPDATE users SET foo = ?, bar = ?, baz = ? WHERE id_tipo_veiculo = ?',
  ['a', 'b', 'c', userId]);
console.log(sql); // UPDATE users SET foo = 'a', bar = 'b', baz = 'c' WHERE id_tipo_veiculo = 1
```
...
console.log(sql); // INSERT INTO posts SET `id_tipo_veiculo` = 1, `title` = 'Hello MySQL'
```
...
var sql = SqlString.format('UPDATE posts SET modified = ? WHERE id_tipo_veiculo = ?', [CURRENT_TIMESTAMP, 42]);
...
var sql     = SqlString.format('SELECT ?? FROM ?? WHERE id_tipo_veiculo = ?', [columns, 'users', userId]);
console.log(sql); // SELECT `username`, `email` FROM `users` WHERE id_tipo_veiculo = 1
```
...
var sql    = SqlString.format('UPDATE ?? SET ? WHERE `id_tipo_veiculo` = ?', ['users', data, userId]);
console.log(sql); // UPDATE `users` SET `email` = 'foobar@example.com', `modified` = NOW() WHERE `id_tipo_veiculo` = 1
```
