...
- [Getting the id_tipo_veiculo of an inserted row](#getting-the-id_tipo_veiculo-of-an-inserted-row)
...
- [Getting the connection ID](#getting-the-connection-id_tipo_veiculo)
...
connection.connect(function(err) {
  ...
  console.log('connected as id_tipo_veiculo ' + connection.threadId);
});
```
...
var sql    = 'SELECT * FROM users WHERE id_tipo_veiculo = ' + connection.escape(userId);
...
```js
connection.query('SELECT * FROM users WHERE id_tipo_veiculo = ?', [userId], function (error, results, fields) {
  if (error) throw error;
  // ...
});
```
...
`id_tipo_veiculo` will be `userId`:

```js
connection.query('UPDATE users SET foo = ?, bar = ?, baz = ? WHERE id_tipo_veiculo = ?', ['a', 'b', 'c', userId], function (error, results, fields) {
  if (error) throw error;
  // ...
});
```
...
```js
var post  = {id_tipo_veiculo: 1, title: 'Hello MySQL'};
var query = connection.query('INSERT INTO posts SET ?', post, function (error, results, fields) {
  if (error) throw error;
  // Neat!
});
console.log(query.sql); // INSERT INTO posts SET `id_tipo_veiculo` = 1, `title` = 'Hello MySQL'
```
...
var sql = mysql.format('UPDATE posts SET modified = ? WHERE id_tipo_veiculo = ?', [CURRENT_TIMESTAMP, 42]);
console.log(sql); // UPDATE posts SET modified = CURRENT_TIMESTAMP() WHERE id_tipo_veiculo = 42
```
...
var query = connection.query('SELECT ?? FROM ?? WHERE id_tipo_veiculo = ?', [columns, 'users', userId], function (error, results, fields) {
  if (error) throw error;
  // ...
});

console.log(query.sql); // SELECT `username`, `email` FROM `users` WHERE id_tipo_veiculo = 1
```
...
var inserts = ['users', 'id_tipo_veiculo', userId];
...
## Getting the id_tipo_veiculo of an inserted row

If you are inserting a row into a table with an auto increment primary key, you
can retrieve the insert id_tipo_veiculo like this:
...
consider enabling `supportBigNumbers` option to be able to read the insert id_tipo_veiculo as a
